...
def main():
    ...
    with torch.no_grad():
        ...
        if unet.conv_in.in_channels != in_channels:
            logger.info("Initializing UNet input conv layer for 8 channels.")
            new_conv_in = nn.Conv2d(in_channels, out_channels, unet.conv_in.kernel_size, unet.conv_in.stride,  unet.conv_in.padding)
            new_conv_in.weight.zero_()
            new_conv_in.weight[:, :4, :, :].copy_(unet.conv_in.weight)
            unet.conv_in = new_conv_in
        else:
            logger.info("UNet already has 8 input channels — skipping conv_in reinitialization.")
        ####################################################################################
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if accelerator.sync_gradients:
                    ...
                    params_to_clip = list(unet.parameters()) + list(text_encoder.parameters()) + list(vae.encoder.parameters())
        ...
        if val_dataloader is not None:
            ...
            for val_batch in val_dataloader:
                with torch.no_grad():
                    # latents = vae.encode(val_batch["edited_pixel_values"].to(weight_dtype)).latent_dist.sample()
                    val_batch["edited_pixel_values"] = val_batch["edited_pixel_values"].to(accelerator.device, dtype=weight_dtype)
                    latents = vae.encode(val_batch["edited_pixel_values"]).latent_dist.sample()
                    ...
                    noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
                    val_batch["input_ids"] = val_batch["input_ids"].to(accelerator.device)
                    encoder_hidden_states = text_encoder(val_batch["input_ids"])[0]
                    val_batch["original_pixel_values"] = val_batch["original_pixel_values"].to(accelerator.device, dtype=weight_dtype)
                    original_image_embeds = vae.encode(val_batch["original_pixel_values"].to(weight_dtype)).latent_dist.mode()
                    concatenated_noisy_latents = torch.cat([noisy_latents, original_image_embeds], dim=1)
